...
* 调度方式上。协程是用户态，有自己的调度器。线程是操作系统调度。
* 上下文切换速度上。线程要经过操作系统用户态与内核态的切换，协程不用。
* 栈的大小上。线程栈一般相对较大（比如2MB)，且不能扩容。协程小很多，只有2KB，且能动态扩容。

**协程的几种状态：**

![goro_state](images/goro_state.svg)



**特殊协程g0：**

用来调度一般的g。g   ----> go  ---- > g，就是一次调度。



**调度策略：**

![goro_sched](images/goro_sched.svg)

先从本地runq（局部runq)查找g，再从全局runq查找，为保证全局runq能被执行，每调度61次，就优先从全局runq取一个g运行。如果全局runq还是没有，就从准备就绪的网络列表里找，如果还是没有，就从其他P里偷一半过来。
...
由于标记协程与用户协程是并发进行的，所以可能出现标记协程完成了黑色对象的扫描后，用户协程将黑色对象引用到了白色对象，而这个白色对象没有其他的灰色路径引用。就会造成对象被回收。

屏障正是解决这样的问题。在用户协程修改对象引用的时候，插入一个操作，直接将引用对象置灰，或者通过其他的灰色路径引用到它，维持强弱三色不变性。
